...
higgs_decay_id = {
        ...
        "H→Zγ": 20, "H→gluons": 21, "H→γγ": 22, "H→ZZ*": 23, "H→WW": 24,
    }
...
def data_to_matrix(matrix_data):
    br_to_classes = np.zeros((len(matrix_data["data_class"].cat.categories),
...
def get_train_test_matrices(rootfile_path):
    ...
    matrix_data["BR"] = pd.Categorical(df["hDecay"], ordered=True,
                              categories=sorted(higgs_decay_id.values()),
                            ).rename_categories(higgs_decay_name)
    train_df, test_df = train_test_split(matrix_data, test_size=0.5,
        random_state=42, stratify=matrix_data["BR"])
